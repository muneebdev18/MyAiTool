...
function enhancePrompt(userPrompt) {
  ...
  if (selectedStyle === 'detailed') {
    return `A high-quality, detailed image of ${userPrompt}. The image should be vivid, clear, and focus on accurately representing the subject.`;
  } else if (selectedStyle === 'animated') {
    return `A colorful and animated style image of ${userPrompt}.`;
  }
}
...
async function generateImages() {
  ...
  imageContainer.innerHTML = '<div class="spinner flex justify-center"></div>';
  errorLog.innerHTML = '';
  ...
  try {
    ...
    for (let i = 0; i < imageCount; i++) {
      ...
      requests.push(axios.post('/api/generate-image', { prompt: enhancedPrompt },{ headers: {
